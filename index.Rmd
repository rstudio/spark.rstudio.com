...
<!-- ![](images/ide-connect-dialog.png){#ide-screenshots .pull-left} -->

<div class="lead" id="intro-p">

- Connect to Spark from R; sparklyr implements a complete [dplyr](https://github.com/hadley/dplyr) backend.
- Interact with massive Spark datasets from an interactive R session.
- Filter and mutate Spark datasets and bring them into R for further analysis and visuzliation.
- Leverage Spark's MLlib machine learning techniques without ever downloading a Spark dataset to your R session.
...
If you're just getting started with Spark, sparklyr provides a way to run a Spark cluster on your local machine; in this mode, you don't need any additional servers or infrastructure. Alternatively, you can use sparklyr to connect to an existing cluster.

Below, you can find how to connect to either a local or a cluster. If you're using the RStudio IDE, you can click "New Connection" in the `Spark` tab and have it produce this code for you.
...
If you're running Spark locally, you can run whatever version of Spark you want with any available version of the Hadoop libraries so long as those versions have been installed using `spark_install()` previously.
...
If you're connecting to a cluster, it's important to make sure that the your targeted Spark version and the version of the Hadoop libraries that you use align with what the cluster is using.
