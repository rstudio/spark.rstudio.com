...
<img src="images/sparklyr_ref/sparklyr_ref_gradient.png" id="lead-logo" width=400 height=218 class="pull-right"/>
...
You can copy R data frames into Spark using the [copy_to](reference/sparklyr/latest/copy_to.html) function. More typically though you'll read data within the Spark cluster using the [spark_read](reference/sparklyr/latest/spark_read_parquet.html) family of functions:

```{r}
iris_tbl <- copy_to(sc, iris)
flights_tbl <- spark_read_csv(sc, "flights", "hdfs://hdfs.net:9000/path/flights.csv")
batting_tbl <- spark_read_parquet(sc, "batting", "s3n://bucket/path/batting.parquet")
```
