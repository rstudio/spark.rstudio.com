...
When doing simple mathematical operations of the form you normally use when filtering, mutating and summarising it’s relatively straightforward to translate R code to SQL (or indeed to any programming language). plyr knows how to convert the following R functions to Spark SQL:
...
```{r}
flights %>%
  mutate(x1 = dep_delay %% 2) %>%
  mutate(x2 = floor(dep_delay)) %>%
  mutate(x3 = arr_delay > dep_delay && sched_arr_time > sched_dep_time) %>%
  select(arr_delay, dep_delay, x1:x3)
```
```
Source:   query [?? x 5]
Database: spark connection master=local app=sparklyr local=TRUE
