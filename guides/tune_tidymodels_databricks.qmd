...
Tidymodels. Instead of calling `tune_grid()` simply call `tune_spark_grid()` and
`sparklyr` will take care of all the setup and parallel job scheduling inside
Databricks. The output from `tune_spark_grid()` is the exact same type of R
object `tune_grid()` returns. 
