...
back-end, and RStudio's Connection pane integration.
...
To access Spark Connect, you will need to install the `pysparklyr` package in
addition to `sparklyr`: 
...
`sparklyr` will automatically set up a temporary Python environment for
the duration of your R session using `uv`. This environment will
include all the Python libraries needed to interact with the Spark
cluster. No manual installation steps are required, simply call
`spark_connect()` and everything will be ready to use.

### Legacy setup via `install_pyspark()`

If `uv` is not available in your environment (for example, because your
organization restricts its installation), you can fall back to setting
up the Python environment manually using `install_pyspark()`. This will:

- Create, or re-create, a persistent Python environment. Based on your
OS, it will choose to create a Virtual Environment, or Conda.
...
It is recommended that the version of the PySpark library matches the
Spark version of your cluster. To do this, pass the Spark version in
the `version` argument, for example:

```r
pysparklyr::install_pyspark("4.1")
```

We have seen Spark sessions crash when the version of PySpark and the
version of Spark do not match. Specifically when a newer version of
PySpark is used against an older version of Spark. If you are having
issues with your connection, consider running `install_pyspark()` to
match the cluster's specific Spark version.
...
address and an optional port. The connection URL uses the "sc://" protocol. For `method`, use
...
It is possible to run Spark Connect on your machine. We provide helper
...
Starting with Spark 4.0, Spark Connect has access to the full
[MLlib library](https://spark.apache.org/docs/latest/api/python/reference/pyspark.ml.html).
This means you can now train and apply machine learning models directly
from R via `sparklyr`.

Here is an example that fits a linear regression model on the same
Spark DataFrame:
...
Spark Connect models have a specialized print output that differs from
other `sparklyr` deployment types. It displays a summary of the model,
including the coefficients and fit statistics such as RMSE, R², and
mean absolute error:
...
For a full list of the supported MLlib functions in `sparklyr`, see the
[appendix](#appendix-supported-mllib-functions).
...
The following `sparklyr` MLlib functions are supported via Spark Connect
(requires Spark 4.0+):
