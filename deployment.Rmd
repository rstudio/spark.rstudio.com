...
```{r}
sparklyr::spark_install(version = "2.0.0", hadoop_version = "2.6")
```
...
| Option | Description  |
|---------------|----------------------------------------------------------|
| `spark.*` | Configuration settings for the Spark context (applied by creating a `SparkConf` containing the specified properties). For example, `spark.executor.memory: 1g` configures the memory available in each executor [Spark Configuration](see http://spark.apache.org/docs/latest/configuration.html#application-properties) for additional options. |
