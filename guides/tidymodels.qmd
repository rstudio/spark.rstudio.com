...
learning. Just like `sparklyr`, `tidymodels` uses `tidyverse` principles.
...
-   `augment()` - Adds the prediction columns to the data set. This function 
     is similar to `ml_predict()`, but instead of returning only a vector of predictions 
     (like `predict()`), it adds the new column(s) to the data set.
    `augment()` 
