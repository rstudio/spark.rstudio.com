...
RStudio Connect, and RStudio Package Manager to work with Databricks.
...
- **Connect to Databricks remotely** - Work with Databricks as a remote compute
  ...
  accomplished using Databricks Connect as described in the
  [Connecting to Databricks remotely](#option-1---connecting-to-databricks-remotely)
  section below or by performing SQL queries with JDBC/ODBC using the Databricks
  Spark SQL Driver on
...
support RStudio Connect or the process sandboxing mechanisms that it uses for
published content.
...
There are three options for using RStudio Connect with Databricks:

1. Connecting to Databricks remotely and using `sparklyr` with
   [Databricks Connect](https://docs.databricks.com/dev-tools/databricks-connect.html)
   (Recommended Option)
2. Performing SQL queries with JDBC/ODBC using the Databricks Spark SQL Driver
   on [AWS](https://docs.databricks.com/integrations/bi/jdbc-odbc-bi.html) or
   [Azure](https://docs.microsoft.com/en-us/azure/databricks/integrations/bi/jdbc-odbc-bi)
   (Alternative Option)
3. Adding calls in your R code to create and run Databricks jobs
   [via the Databricks Jobs API](https://github.com/RafiKurlansik/bricksteR/)
   (Alternative Option)
