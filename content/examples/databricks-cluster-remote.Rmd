...
- RStudio Server Pro
- Java 8 installed on the server with RStudio Server Pro
- Active Databricks cluster with a runtime version 5.5 and above

## Install Python

The Databricks Connect client is provided as a Python library.

Follow the steps to install Python on the server with RStudio Server Pro:

https://docs.rstudio.com/resources/install-python/

Note that you can either install Python for all users in a global location or
for an individual user in your home directory.
...
the `pip --user` option or installing into a conda environment or virtual
environment).
...
Note: The functionality to use `sparklyr` with Databricks Connect is under
active development and is currently only available in the latest development
version.

To use `sparklyr` with Databricks Connect, you can install the development
version of `sparklyr` by running the following R code:
...
You can install Spark by running the following R code:
...
Note that the `SPARK_HOME` environment variable must be set to the output of
the `databricks-connect get-spark-home` command.

You can set the `SPARK_HOME ` environment variable and connect to the Databricks
cluster by running the following R code:
...
For more information on the setup, configuration, and limitations of Databricks
Connect, refer to the
[Databricks Connect page in the Databricks documentation](https://docs.databricks.com/dev-tools/databricks-connect.html).
