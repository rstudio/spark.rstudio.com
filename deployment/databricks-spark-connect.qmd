...
To connect to your local Spark Connect, use **localhost** as the address
for `master`:
...
-   **Most `tidyr` functions** - This is ongoing work that we are
    focusing on in `sparklyr`. We are implementing these functions using
    PySpark DataFrame commands, instead of depending on the Scala
    implementation.
