...
It is now possible to take the exact same `tidymodels` objects created locally, 
and have the tuning run in Spark. With one function call, `sparklyr` will upload 
the `tidymodels` objects,  and have the parallel jobs run remotely in the Spark
session.
...
Instead of calling `tune_grid()` use `tune_grid_spark()`. 
The function accepts the exact same arguments as `tune_grid()`. The only required
added argument is the Spark session.

### Why?

**It is important to note that we are not speaking of "big data", but rather of something more like 
...
we have to train on gigabytes and gigabytes of data. The main advantage is being
able iterate faster. Having to wait less time for results, allows us to possibly
...
The minimum requirements to tune the model in Spark are:
...
There are passed to a function in `sparklyr` called `tune_grid_spark()`.
