...
- A new [reactiveSpark()](#example-4---shiny-integration) function, that allows Shiny apps to poll the contents of the stream
...
<center><p><img src="/guides/streaming/shiny.png" align='center' width = 500/></p></center>
...
1. This very basic Shiny app simply displays the output of a table in the `ui` section
    ```{r, eval = FALSE}
    library(shiny)
...
This example uses a fitted [Pipeline Model](/guides/pipelines/) to process the input, and saves the predictions to the output.  This approach would be used to apply Machine Learning on top of streaming data.
...
1. This example pipelines the input, process and output in a single code segment.  The `ml_transform()` function is used to create the predictions.  Because the CSV format does not support *list* type fields, the `features` column is removed before the results are sent to the output.
    ```{r, eval = FALSE}
    ml_stream <- stream_read_csv(
        sc = sc, 
        path = "source", 
