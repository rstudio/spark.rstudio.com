...
If `columns` is not passed, then you will receive a message that starts:
"To increase performance...", and will provide you the spec it used for
your `spark_apply()` call. You can copy and use if you plan to run the
...
Passing a `columns` spec will make`spark_apply()` run faster. If not
provided, `sparklyr` has to collect the first 10 rows, and run the R
code in order to try and determine the names and types of your resulting
data set.
