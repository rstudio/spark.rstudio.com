...
14.0.  It will automatically name it `r-sparklyr-databricks-14.0`.  This way
it will allow `sparklyr` choose the Python environment with the right 
library version. 
...
It is recommended that the version of the PySpark library matches 
the Spark version of your cluster.  To do this, pass the Spark version in the 
`version` argument, for example:
...
We have seen where a newer version of PySpark will crash when used against an
older version of Spark. If you are having issues with your connection, 
definitely consider running the `install_pyspark()` for the cluster's specific
version.
