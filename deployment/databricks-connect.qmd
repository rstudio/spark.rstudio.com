...
ways that you can let `deploy_databricks()` know the path to use:
...
-   The cluster's ID can also be used and `pysparklyr` will automatically determine the required DBR
    version:
...
-   If you just ran the code of the content you plan to deploy, then the
   ...
   `deploy_databricks()` will validate that the path of that Python environment
    conforms to one that `pysparklyr` creates and use that. This will
    happen if no `version`, `cluster_id`, or `python` argument is provided. At that
    point simply run:
...
prompted to confirm that the information gathered is
...
default but allow you to easily change it if necessary:
