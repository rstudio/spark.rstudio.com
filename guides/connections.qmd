...
::: callout-note
Spark 4.0 deprecated the **SparkR** package (the official R API maintained by the Apache Spark project). This does not affect `sparklyr`, which uses its own independent interface to Spark and remains fully supported.
:::
...
The [Running on YARN](https://spark.apache.org/docs/latest/running-on-yarn.html) page in Spark's official website is the best place to start for configuration settings reference, please bookmark it. Cluster administrators and users can benefit from this document.
...
Using `"yarn"` as the value for the `master` argument in `spark_connect()` defaults to client mode, making the server where R is running the Spark session driver. Here is a sample connection:
...
Running in cluster mode means that YARN will choose where the driver of the Spark session will run. This means that the server where R is running may not necessarily be the driver for that session. See [Running Spark on YARN](https://spark.apache.org/docs/latest/running-on-yarn.html) in the official Spark documentation for further details.
...
Sys.setenv(JAVA_HOME="/usr/lib/jvm/java-17/")
