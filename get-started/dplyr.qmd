...
tbl_airlines <- copy_to(sc, nycflights13::airlines, "airlines")
...
```{r}
select(tbl_flights, year:day, arr_delay, dep_delay)
```


```{r}
filter(tbl_flights, dep_delay > 1000)
```

```{r}
arrange(tbl_flights, desc(dep_delay))
```
...
```{r}
mutate(tbl_flights, speed = distance / air_time * 60)
```
...
```{r}
tbl_flights %>% 
  left_join(tbl_airlines, by = "carrier") %>% 
