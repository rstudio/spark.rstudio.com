...
The previous example can be re-written to be more compact and clear using R's native pipe operator (`|>`):
...
Accepting a `spark_jobj` or `spark_dataframe` as the first argument of a function makes it very easy to incorporate into pipe-based workflows so this pattern is highly recommended when possible.
