...
- **Connect to Spark remotely** - Install RStudio Server Pro outside of the
  Databricks cluster and work with Spark remotely
- **Work across RStudio Team** - Install RStudio Connect and RStudio Package
  Manager outside of the Databricks cluster so that they are not limited to the
  resources or ephemeral nature of Databricks clusters
...
- **Load and query results efficiently** - Shiny apps that use Spark on the
  backend tend to have performance and runtime issues; instead, consider reading
  the results from a database, file system, or cloud storage instead
- **Maintain a persisent installation** - If you cannot work with Spark
...
- Option 1:
  ...
  (Recommended option)
- Option 2:
  [Working with Databricks locally](#option-2---working-with-databricks-locally)
  (Alternative option)
