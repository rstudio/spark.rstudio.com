...
Spark and `sparklyr` provide an easy way to go from exploration, to
modeling, to tuning. Even without a "formal" Spark cluster, it is
possible to take advantage of these capabilities right from our personal
computer.
...
models across the cluster will cut down the tuning processing time. The
resources available to the cluster, and the given Spark session, will
also determine the the amount of time saved.
...
For example, suppose we are going to try two parameters. For the first
parameter we provide 5 values to try, and for the second parameter we
provide 10 values to try. The total number of combinations will be 50.
The number of combinations grows quickly the more parameters we use. In
our example, adding a third parameter, with only 2 values, our number of
combinations would double to 100 (5x10x2).

The model tuning will then return a performance metric result for each
combination. We can then compare the results, and decide which
combination we will use for our final model.

In Spark, we use an ML Pipeline model, and a list of the parameters and
the values to try. We also specify the metric it should use to measure
performance. Spark will then take care of figuring out the combinations,
and fits the corresponding models.
...
In the previous section, we only referred to combinations, not a fitted
model. That is because with Cross Validation, multiple models are fitted
with the same combination of parameters. The difference is the data used
for training, and validation. These are called folds. The training data,
and the validation data is different for each fold, that is called
re-sampling.
...
Validation will fit, and validate, 375 models total.

In Spark, running the 375 discrete models can be distributed across the
entire cluster, thus significantly reducing the amount of time we would
...
The [Text Modeling](textmodeling.qmd){target="_blank"} article, the data
preparation and modeling are based on the same example from `tune`.
Thanks to how modular ML Pipelines are, we are able to split the
modeling walk-through, from the tuning walk-through found in this
article. We also recommend to be familiar with the concepts from [Intro
to Model Tuning](model_tuning.qmd){target="_blank"}.
