...
To do this simply call `tune_grid_spark()` instead of `tune_grid()`.
...
shows how the same parsnip model, recipe and re-samples can be passed
...
combination of re-samples and tuning parameters  In the example, we used the
...
are then combined with the 10 folds/re-samples, giving us 100 total 
...
Despite the number of re-samples, or number of tuning +
