...
The following functions implement interfaces for various MLlib algorithms:

| Function | Description  |
|----------------------------|---------------------------------------------|
| [`ml_kmeans`](reference/sparklyr/latest/ml_kmeans.html) | K-Means Clustering |
| [`ml_linear_regression`](reference/sparklyr/latest/ml_linear_regression.html) | K-Means Clustering |
...
| [`ml_gradient_boosted_trees`](reference/sparklyr/latest/ml_gradient_boosted_trees.html) | Gradient-Boosted Tree |
...
Here's an example that uses the `mtcars` data set to build a model. First we will initialize the Spark connection and copy the data into Spark.
...
The following functions implement interfaces to various MLlib feature transformers:
...
A simple example is the binarizer, which transforms a feature into 1's and 0's based on a threshold. We can set compare 8 cylindar vehicles to all other types of vehicles with the following:
