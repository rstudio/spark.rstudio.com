...
A typical machine learning pipeline with **sparklyr** might be composed of the following stages. Given a Spark DataFrame, you might need to:
...
4. Inspect and use your model fit to make further predictions on new data.

Let's walk through a motivating example to see how these pieces might come together. We'll use `ml_linear_regression` to fit a linear regression model. We'll use the built-in `mtcars` dataset, and see if we can predict a car's fuel consumption (`mpg`) based on its weight (`wt`), and the number of cylinders the engine contains (`cyl`). We'll assume in each case that the relationship between `mpg` and each of our features is linear.
...
data <- data.frame(
  ...
  actual    = sdf_collect(partitions$test)$mpg
)

# plot predicted vs. actual values
ggplot(data, aes(x = actual, y = predicted)) +
  geom_point() +
  geom_abline(lty = "dotted", col = "red") +
...
Our model appears to do a fairly good job of predicting a car's average fuel consumption.
...
A model is often fit not on the columns of data within a dataset as-is, but instead on some transformation of those columns. Spark provides a large number of algorithms for transforming features in a Spark DataFrame, and **sparklyr** exposes these within the `ft_*` family of functions. These routines generally take one or more input columns and generate a new output column formed as a transformation of those columns.
...
sdf_register(spark_df, "mtcars_mutated")
```
```
Source:   query [?? x 12]
...
`sdf_mutate()` is provided as a helper function, to allow you to use feature transformers within a pipeline. For example, the previous code snippet could have been written as:

```{r, eval=FALSE}
mtcars_tbl %>%
  mutate(wt1000 = wt * 1000) %>%
  sdf_mutate(cyl8 = ft_binarizer(cyl, 6)) %>%
  sdf_register("mtcars_mutated")
```
```
Source:   query [?? x 12]
Database: spark connection master=local app=sparklyr local=TRUE
...
It's important to understand that the `sdf_*` and `ft_*` family of functions return a lower-level Spark DataFrame wrapper object, rather than the `tbl_spark` that you might already be familiar with. These functions differ in their behavior from the `dplyr` interface in a few important ways:

1. Operations on a `tbl_spark` (through the dplyr interface) operate entirely through the Spark SQL interface, while these functions often directly access the Spark DataFrame API;

2. dplyr functions return their results lazily (the generated SQL is not evaluated until you explicitly `collect()` the result), while these functions eagerly generate new Spark DataFrames;

3. A `tbl_spark` has a name in the underlying Spark SQL context, thereby allowing you to query that table in SQL queries; the lower-level Spark DataFrames returned do not have an associated table name.

If you need to transform a Spark DataFrame back into a `tbl_spark` within a pipeline, you can use `sdf_register()` to assign it a new name, and return that Spark DataFrame as a `tbl_spark`.
