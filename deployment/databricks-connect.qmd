...
The API is very different than the "legacy" Spark and using the Spark shell
...
`dplyr`back-end, `DBI` back-end, and integration with RStudio's Connection pane.
...
package, called [`pysparklyr`](https://github.com/mlverse/pysparklyr), is an extension of `sparklyr`.
...
3.  Your
    [Cluster ID](https://docs.databricks.com/workspace/workspace-details.html#cluster-url-and-id)

We have developed this solution to align with other
applications that integrate with Databricks. All applications need, at
minimum, a work space *(1)*, and an authentication token *(2)*. For
...
thing that will change more often is the cluster you are connecting to. Using
environment variables also makes connection safer, because token contents will
not be in your code in plain text. We recommend that you set these two variables
at your **user level**. To do this run:
...
-   Create or re-create the necessary Python environment. Based on your OS, it
    will choose to create a Virtual Environment or use Conda.
...
expanding from the top level, all the way down to the table you wish to explore.
Once expanded, the table's fields and their types are displayed.

![](/images/deployment/connect/rstudio-connection.png)

You can also click on the **table** icon, situated to
...
**trips** table, which is inside **nyctaxi** database, which is inside the
***samples** catalog:
...
After pointing `tbl()` to that specific table, you can then use `dplyr` to
...
libraries that relate to Machine Learning. These include Torch and
"scikit-learn". Some of the libraries are large in size and they may have Python
requirements that are challenging to new users. Additionally, we have noticed
there are not many users that need to utilize ML capabilities at this time.
...
the issues that have been reported and the recommended solution
...
as of today 14.2, is ahead of the `databricks.connect` version
available in `databricks.connect`.

### Solution

Current workaround is to say "No" when prompted to install version 14.2 and, if
not already installed, install 14.1 using:
...
need to connect to a different cluster that has the same DBR
...
-   **ML functions** - Very few functions, in `sparklyr`, that have the
    `ml_` and `ft_` are currently supported. The reason is that
