...
**IMPORTANT NOTE**: Extension packages should restrict their use of functions within sparklyr to those listed within this guide. This is because we plan to factor these functions out into a separate package that defines the core Spark extension API. This will it possible in principle for extension packages to work seamlessly with multiple Spark front-end packages (e.g both sparklyr and [SparkR](https://spark.apache.org/docs/latest/sparkr.html)).
...
The `count_lines` function takes a Spark connection (`sc`) argument, which when passed to `spark_invoke` enables executing methods of the SparkContext object. Creating new objects also requires the `sc` argument.
...
| Function | Description  |
|----------------------------|---------------------------------------------|
| [`spark_dependency`](reference/sparklyr/latest/spark_dependency.html) | Define a Spark dependency consisting of JAR files and Spark packages. |
