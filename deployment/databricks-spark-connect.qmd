...
Connect. For now, to access the new capabilities, you will need the
development versions of the `sparklyr`, and `pysparklyr`, packages.
These versions are available in their respective GitHub repositories. To
install use:
...
you will need to set the `host`, and `method`. The `host` will be an IP,
...
It is possible to test Spark Connect inside your machine. To do so,
download Spark version 3.4 or above, and follow the instructions in this
[article](https://spark.apache.org/docs/latest/spark-connect-overview.html#download-and-start-spark-server-with-spark-connect),
to learn how to start and stop the service.

Once started, you can start a session with Spark Connect using the
following code:
...
-   **`tidyr`** - This is ongoing work that we are focusing on in
    ...
    implementation. **Update** - `pivot_longer()` is now available,
    the 'ptypes', 'transform', and 'names_repair' arguments are
    not supported yet. 
