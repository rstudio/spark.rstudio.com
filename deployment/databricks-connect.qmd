...
project, or it is at the root level of your project. There are three
ways that you can let `deploy_databricks()` the path to use:

-   If you are in RStudio, and the document you wish to publish is open,
    `deploy_databricks()` will use the RStudio's API to get the path of
    that document, and then use its containing folder as the content's
    location. Our hope is that for most time, this will be how you
    deploy.

-   Use the `appDir` argument to pass the folder path to be used.
    Something such as `here::here("my-cool-document")` would work.

-   If no document is opened in RStudio, and `appDir` is left empty,
    then whatever `getwd()` returns is what will be used

The aim of of the new function is to be as flexible, and helpful. It
will gather your document location, credentials, and URLs. The
**Python** location is what you will need to define at the time of
calling `deploy_databricks()`. If you are in RStudio, and your document
is opened, here are several ways to use:
...
-   The cluster's ID can also be used, so `pysparklyr` pulls the DBR
    version:
...
-   If you just ran the code of the content you plan to deploy, then the
    **Python** environment should already be loaded in the R session.
    The function will validate that the path of that Python environment
    conforms to one that `pysparklyr` creates, and uses that. This will
    happen if no `version`, `cluster_id`, or `python` is passed. At that
    point simply run:
...
prompted to confirm that the information that it was able to gathered is
correct. Also notice, that if you have more than one Posit Connect
server setup in your RStudio IDE, it will choose the top one as the
default, and let you easily change it by selecting, in this case 3:
