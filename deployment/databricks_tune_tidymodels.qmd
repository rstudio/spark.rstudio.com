...
variable. The optional `control_grid()` call is included here to display the 
`sparklyr` output during the tuning process.
...
Library installation is a straightforward process managed through the
Databricks web portal. Here are the instructions that show you how to do
that: [Databricks - Cluster
...
the user has rights to modify the libraries in the target cluster.
