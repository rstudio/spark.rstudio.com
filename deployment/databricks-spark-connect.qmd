...
`reticulate` will use `RETICULATE_PYTHON` as the first way to find the Python
installation to use, see [`reticulate` - Order of Discovery](https://rstudio.github.io/reticulate/articles/versions.html#order-of-discovery).
This means that any Python environment you created, via `install_databricks()` or 
otherwise, will not be used. Full message says:

```r
# Warning message:
# The request to `use_python("[path to your new environment]")` will be ignored because the  environment variable RETICULATE_PYTHON is set to "[whatever that variable is set to]" 
```


### Solution

The workaround is to unset the environment variable. This will allow `reticulate`
to use the your newly created Python environment:

```r
library(sparklyr)

# You will need to do this every time you connect
...
::: {.callout-note collapse="true"}
## The RStudio IDE Connections Pane is stuck on "Loading Objects"

The RStudio IDE Connections Pane is stuck on "Loading Objects", additionally,
the connection seems to have been successful, but no catalogs or tables show 
up in the Connections Pane. 

The likely root cause is that you have access to a vast amount of catalogs, 
databases, and tables.  This issue is reported most often by Databricks 
internal users. 

### Solution

The best workaround we have found is to load the Unity Catalog data into the
Connections Pane. In other words, we will filter out the content from the
Hive Metastore (HMS). To do this, `pysparklyr` will look at a new environment
variable called: `SPARKLYR_RSTUDIO_CP_VIEW`, to "uc_only":
...
If you prefer, you can set this variable at your profile or project's level.

To access this functionality, you will need `pysparklyr` version **0.1.9001** or
